// anpag/openmaptest/openmaptest-66d4ad2d57a47131c6120825c7a749c484b9064d/components/MapDisplay.tsx
...
const MapUpdater: React.FC<{ geoJsonFeature: Feature | null }> = ({ geoJsonFeature }) => {
  const map = useMap();
...
const MapDisplay: React.FC<MapDisplayProps> = ({ geoJsonFeature, isLoading, searchAttempted }) => {
  ...
  const showOverlay = isLoading || !searchAttempted || (searchAttempted && !geoJsonFeature);
  ...
  return (
    <div className="relative w-full h-full bg-gray-300">
      <MapContainer center={defaultCenter} zoom={defaultZoom} scrollWheelZoom={true} className="w-full h-full">
        ...
        />
        {geoJsonFeature && <GeoJSON key={geoJsonFeature.properties?.id} data={geoJsonFeature} style={geoJsonStyle} />}
        <MapUpdater geoJsonFeature={geoJsonFeature} />
      </MapContainer>
           ...
           <div className="text-center text-white p-8 bg-gray-900 bg-opacity-70 rounded-lg shadow-2xl">
            {isLoading ? (
               <>
                ...
                <h2 className="text-2xl font-bold">Fetching Boundary...</h2>
              </>
            ) : !searchAttempted ? (
              <>
                ...
                <p className="mt-2 text-lg">Enter a UK postcode district to begin.</p>
              </>
            ) : (
                <>
                <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                ...
                <h2 className="text-2xl font-bold">Boundary Not Found</h2>
                <p className="mt-2 text-lg">Please check the postcode and try again.</p>
              </>
            )}
          </div>
        </div>
